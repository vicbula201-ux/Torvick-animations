...
de completa en 5s que en 30s, solo que más rápida o más lenta.`;
  ...
  function compilar(src) {
    ...
    var fabrica = new Function(
      'React', 'P', 'M', 'animate', 'Easing', 'interpolate', 'clamp',
      out + '\nreturn typeof Escena === "function" ? Escena : null;'
    );
    var Escena = fabrica(
      global.React, global.P, global.M,
      global.animate, global.Easing, global.interpolate, global.clamp
    );
    if (!Escena) {
  ...
  function Lienzo(props) {
    ...
    if (!Escena) {
      return <div className="vacio">escribe una frase y genera</div>;
    }
    ...
    try {
      ...
      contenido = Escena({ T: T, at: 0, dur: total });
    } catch (e) {
    ...
    return (
      <div style={{ position: 'absolute', inset: 0 }}>
        <div style={{
          ...
          {error ? null : contenido}
        </div>
        {error ? (
          <div className="banda">
            La escena falló al dibujarse: {error}
            <br />Pedí un ajuste describiendo el problema, o editá el código abajo.
          </div>
        ) : null}
      </div>
    );
  ...
  var MODELOS = [
    { id: 'claude-opus-5', nombre: 'Opus 5' },
    { id: 'claude-sonnet-5', nombre: 'Sonnet 5' },
    { id: 'claude-haiku-4-5-20251001', nombre: 'Haiku 4.5' }
  ];

  function App() {
    var _k = useState(function () {
      try { return localStorage.getItem('claveApi') || ''; } catch (e) { return ''; }
    });
    var apiKey = _k[0], setApiKey = _k[1];

    var _m = useState(function () {
      try { return localStorage.getItem('modelo') || MODELOS[0].id; }
      catch (e) { return MODELOS[0].id; }
    });
    var modelo = _m[0], setModelo = _m[1];

    var _s = useState(12);
    var segundos = _s[0], setSegundos = _s[1];

    var _f = useState('');
    var frase = _f[0], setFrase = _f[1];

    var _p = useState('');
    var pedido = _p[0], setPedido = _p[1];

    var _c = useState('');           // codigo fuente actual de la escena
    var codigo = _c[0], setCodigo = _c[1];

    var _b = useState('');           // borrador editable del panel de codigo
    var borrador = _b[0], setBorrador = _b[1];

    var _e = useState(null);         // funcion Escena compilada
    var Escena = _e[0], setEscena = _e[1];

    var _l = useState(false);
    var cargando = _l[0], setCargando = _l[1];

    var _x = useState('');
    var error = _x[0], setError = _x[1];

    var _v = useState(false);
    var verCodigo = _v[0], setVerCodigo = _v[1];
    ...
    useEffect(function () {
      try { localStorage.setItem('claveApi', apiKey); } catch (e) {}
    }, [apiKey]);

    useEffect(function () {
      try { localStorage.setItem('modelo', modelo); } catch (e) {}
    }, [modelo]);
    ...
    function aplicar(src) {
      ...
      try {
        ...
        setEscena(function () { return E; });
        setError('');
      } catch (e) {
        // el codigo no compila: mostrarlo y dejar seguir trabajando
        setEscena(null);
        setError('No se pudo compilar el código: ' +
    ...
    function validar() {
      if (!apiKey.trim()) { setError('Falta la API key.'); return false; }
      if (!frase.trim()) { setError('Escribí una frase del guion.'); return false; }
      return true;
    }
    ...
    function ajustar() {
      if (!apiKey.trim()) { setError('Falta la API key.'); return; }
      if (!pedido.trim()) { setError('Describí qué querés cambiar.'); return; }
      setError('');
      setCargando(true);
      var p = pedido.trim();
        ...
        messages: [
          {
            ...
            content: 'Frase: "' + fraseDeLaEscena.current + '"\nDuración: ' +
              durDeLaEscena.current + ' segundos.\nDevolvé solo la función Escena.'
          },
          { role: 'assistant', content: codigo },
          {
            role: 'user',
            content: 'Ajustá SOLO esto, sin rehacer el resto de la escena: ' + p +
              '\nSi el pedido es sobre duración o velocidad, revisá que TODOS los ' +
              'tiempos usen f(fracción) y no segundos sueltos.\nDevolvé de nuevo ' +
              'la función Escena completa, ya corregida.'
          }
        ]
      }).then(function (txt) {
        aplicar(txt);
    ...
    var bandaError = error ? (
      <div className="banda" style={{ position: 'relative' }}>{error}</div>
    ) : null;
    ...
    return (
      <div className="app">
        ...
        <header className="cabecera">
          <div className="marca">Generador de escenas</div>

          <div className="campo">
            <label htmlFor="seg">Segundos</label>
            <input id="seg" type="number" min="2" max="60" step="1" value={segundos}
              onChange={function (e) {
          ...
          <div className="campo">
            ...
            <input id="key" className="llave" type="password" value={apiKey}
        ...
        <div className="cuerpo">
          ...
          <aside className="lateral">
            <div className="grupo">
              <label className="rotulo" htmlFor="frase">Frase del guion</label>
              <textarea id="frase" rows={4} value={frase}
                placeholder="Siempre pospongo lo que importa"
                onChange={function (e) { setFrase(e.target.value); }} />
            </div>

            <button className="primario" onClick={generar} disabled={cargando}>
              {cargando ? 'Generando…' : 'Generar'}
            </button>

            {hayEscena ? <div className="separador" /> : null}

            {hayEscena ? (
              <div className="grupo">
                <label className="rotulo" htmlFor="pedido">¿Algo no te gustó?</label>
                <textarea id="pedido" rows={3} value={pedido}
                  placeholder="la figura está muy chica"
                  onChange={function (e) { setPedido(e.target.value); }} />
                <button className="ajuste" onClick={ajustar} disabled={cargando}>
                  {cargando ? 'Ajustando…' : 'Ajustar'}
                </button>
              </div>
            ) : null}
            ...
            {hayEscena ? (
              <div className="grupo">
                <button className="plano"
                  onClick={function () { setVerCodigo(!verCodigo); }}>
                  {verCodigo ? 'Ocultar código' : 'Ver código'}
                </button>
                <button className="plano" onClick={exportar}>Exportar .jsx</button>
              </div>
            ) : null}
          ...
          <main className="escenario">
            <global.Stage scenes={[{ name: 'escena', dur: segundos }]}>
              <Lienzo Escena={Escena} />
            </global.Stage>
          </main>
        </div>
        ...
        <footer className="pie">
          <div className="pie-barra">
            <span className="rotulo">Código JSX</span>
            <button className="plano"
              onClick={function () { setVerCodigo(!verCodigo); }}
              disabled={!hayEscena}>
              {verCodigo ? 'Colapsar' : 'Desplegar'}
            </button>
            <button className="plano" onClick={recompilar}
              disabled={!hayEscena || !verCodigo}>Recompilar</button>
          </div>
          {verCodigo ? (
            <textarea className="codigo" spellCheck={false} value={borrador}
              onChange={function (e) { setBorrador(e.target.value); }} />
          ) : null}
        </footer>
      </div>
    );
